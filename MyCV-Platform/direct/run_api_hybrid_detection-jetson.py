...
def load_models(device):
    ...
    try:
        # Load YOLO11m
        yolo11m_path = "data-jetson/models/yolo/active/yolo11m.pt"
        ...
        # Load best.pt
        best_pt_path = "data-jetson/models/trained/active/best.pt"
        ...
        # Load SAM2_b
        sam2_path = "data-jetson/models/sam/active/sam2_b.pt"
...
def main():
    ...
    # Load project info from model_info.json for Jetson
    try:
        with open('data-jetson/models/model_info.json', 'r') as f:
    ...
    # Get test images from remote directory for Jetson
    test_images_dir = "data-jetson/input/remote"
    test_images = []
    ...
    for i, image_path in enumerate(test_images, 1):
        ...
        # Create output directories for this user session (Jetson)
        output_dir = f"data-jetson/output/remote/{timestamp}/{user_id}"
    ...
    log_message("📊 Check 'data-jetson/output/remote' for results", 'info')
...
def create_session_summary(test_images, project_name, project_version, session_id=None):
    ...
    for session_key, session_data in sessions.items():
        ...
        # Output directory for this session (Jetson)
        output_dir = f"data-jetson/output/remote/{timestamp}/{user_id}"
            ...
            try:
                ...
                if os.path.exists(best_image_path):
                    if session_id:
                        ...
                        images["best"] = f"http://100.117.234.2:5000/api/download/{session_id}/{fname}"
                    else:
                        images["best"] = f"http://100.117.234.2:5000/api/download/{timestamp}/{user_id}/best/{os.path.basename(best_image_path)}"
