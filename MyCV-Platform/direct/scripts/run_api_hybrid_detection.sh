...
if [ ! -f "run_api_hybrid_detection.py" ]; then
...
# Create remote directory structure (silent)
mkdir -p "${INPUT_DIR}"
mkdir -p "${OUTPUT_DIR}"
mkdir -p "${YOLO_DIR}"
mkdir -p "${BEST_DIR}"
...
# Clean previous results in output directory (silent)
...
if [ $? -eq 0 ]; then
    print_success "✅ $PROJECT_NAME v$PROJECT_VERSION completed successfully"
else
    print_error "❌ $PROJECT_NAME v$PROJECT_VERSION failed"
    exit 1
fi

# cd ../..

# Show results summary (commented out for silent operation)
