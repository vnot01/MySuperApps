...
echo "[1/6] Install systemd-zram-generator if not present"
if ! dpkg -l | grep -q systemd-zram-generator; then
  sudo apt update -y
  sudo apt install -y systemd-zram-generator
fi

echo "[2/6] Write systemd zram generator config (2x RAM, cap 16G, zstd, prio 150)"
...
echo "[3/6] Stop zram service and detach old device"
...
echo "[4/6] Reload systemd and start zram service"
sudo systemctl daemon-reload
sudo systemctl start systemd-zram-setup@zram0.service

echo "[5/6] Ensure NVMe /swapfile active with priority 60"
sudo swapoff /swapfile 2>/dev/null || true
sudo swapon -p 60 /swapfile
sudo sed -i.bak -E 's#^(/swapfile\s+none\s+swap\s+).*#\1sw,pri=60 0 0#' /etc/fstab
grep -qE '^/swapfile\s+none\s+swap' /etc/fstab || \
  echo '/swapfile none swap sw,pri=60 0 0' | sudo tee -a /etc/fstab >/dev/null

echo "[6/6] Persist kernel tunables and verify"
