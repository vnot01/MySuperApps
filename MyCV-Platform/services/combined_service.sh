...
# MyCV-GPU-Server Combined Service Manager
# Mengelola API dan Web Service secara bersamaan
...
api_running() {
    ...
    if [ -n "$api_pid" ] && ps -p "$api_pid" > /dev/null 2>&1; then
        echo "$api_pid" > "/tmp/mycv-gpu-server.pid"
        return 0
    fi
...
stop_all_ports() {
    ...
    # Clean up PID files
    rm -f /tmp/mycv-gpu-server.pid /tmp/mycv_web.pid
...
start_all() {
    print_status "🚀 Starting MyCV-GPU-Server Combined Services..."
    echo ""
...
stop_all() {
    print_status "🛑 Stopping MyCV-GPU-Server Combined Services..."
    echo ""
...
restart_all() {
    print_status "🔄 Restarting MyCV-GPU-Server Combined Services..."
    stop_all
    sleep 3
    start_all
}
...
status_all() {
    print_status "📊 MyCV-GPU-Server Combined Services Status"
    echo "=============================================="
    echo ""
    ...
    if api_running; then
        local api_pid=$(cat "/tmp/mycv-gpu-server.pid" 2>/dev/null || echo "unknown")
    ...
    if web_running; then
        ...
        print_status "  🌐 URL: http://100.98.142.94:5002"
...
show_logs() {
    print_status "📄 MyCV-GPU-Server Combined Services Logs"
    echo "============================================="
    echo ""
    ...
    echo "----------------------"
    if [ -f "/tmp/mycv-gpu-server.log" ]; then
        tail -20 "/tmp/mycv-gpu-server.log"
    else
...
main() {
    case "${1:-}" in
        ...
        remove-autostart)
            ...
            ;;
        *)
            echo "MyCV-GPU-Server Combined Service Manager"
            echo "====================================="
            echo ""
