...
# MyCV-GPU-Server API Service Manager
...
start_api() {
    ...
    print_status "🚀 Starting MyCV-GPU-Server API Service..."
...
stop_api() {
    ...
    local pid=$(cat "$PID_FILE")
    print_status "🛑 Stopping MyCV-GPU-Server API Service (PID: $pid)..."
...
restart_api() {
    print_status "🔄 Restarting MyCV-GPU-Server API Service..."
    stop_api
    sleep 2
    start_api
}
...
show_help() {
            echo "MyCV-GPU-Server API Service Manager"
    echo "=================================="
    echo ""
