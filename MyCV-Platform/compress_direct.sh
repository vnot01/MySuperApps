...
# MyCV-Platform Direct Folder Compressor
# Mengompres folder direct dengan timestamp

echo "📦 MyCV-Platform Direct Folder Compressor"
echo "=========================================="
echo ""
...
if [ ! -d "direct" ]; then
    print_error "Folder 'direct' tidak ditemukan!"
...
ARCHIVE_NAME="MyCV-Platform-Direct-${TIMESTAMP}.tar.gz"
...
if [ $? -eq 0 ]; then
    ...
    echo ""
    print_success "🎉 Folder direct berhasil dikompres!"
    print_success "📦 Archive: ${ARCHIVE_NAME}"
    print_success "💾 Ukuran: ${FILE_SIZE}"
    print_success "📁 Lokasi: ${ARCHIVE_PATH}"
    
else
    print_error "❌ Gagal mengompres folder direct"
    exit 1
fi
