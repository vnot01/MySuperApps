...
function setupEventListeners() {
    ...
    // Initialize menu toggle for dashboard
    initializeMenuToggle();
}

// Initialize menu toggle functionality for dashboard
function initializeMenuToggle() {
    console.log('Initializing menu toggle for dashboard...');
    
    // Get all menu toggles
    const menuToggles = document.querySelectorAll('.layout-menu .menu-toggle');
    console.log('Found menu toggles on dashboard:', menuToggles.length);
    
    // Add click event to each toggle
    menuToggles.forEach((toggle, index) => {
        console.log(`Setting up dashboard toggle ${index + 1}:`, toggle);
        
        // Remove any existing event listeners
        toggle.removeEventListener('click', handleDashboardMenuToggle);
        
        // Add new event listener
        toggle.addEventListener('click', handleDashboardMenuToggle);
    });
}

// Menu toggle handler for dashboard
function handleDashboardMenuToggle(e) {
    e.preventDefault();
    e.stopPropagation();
    
    console.log('Dashboard menu toggle clicked:', this);
    ...
    if (!subMenu) {
        console.log('No sub-menu found on dashboard');
        return;
    }
    
    console.log('Dashboard menu item:', menuItem);
    console.log('Dashboard sub menu:', subMenu);
